...
//max_angle 是sin函数的幅值
static float max_angle = 40;//摇摆原地不动摇摆最大角度(度)
//add_time 是摇摆角度改变的快慢，最大越快
static float const add_time = PI / 280.0f;
/* 底盘定时任务*/
uint16_t fric_speed;
...
//	USART6_Transmit();
//	Ni_Ming(0xf1,chassis.wheel_spd_ref[0],chassis.wheel_spd_ref[1],chassis.wheel_spd_ref[2],chassis.wheel_spd_ref[3]);
//	Ni_Ming(0xf2, chassis.wheel_spd_fdb[0],chassis.wheel_spd_fdb[1],chassis.wheel_spd_fdb[2],moto_chassis[0].speed_rpm);
